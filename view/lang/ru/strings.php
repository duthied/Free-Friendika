...
$a->strings["Post published."] = "Запись опубликована.";
...
$a->strings["Conversation not found."] = "Диалог не найден.";
...
$a->strings["Contact suggestion successfully ignored."] = "Предложенный контакт проигнорирован";
...
$a->strings["Tag(s) removed"] = "Тэги удалены";
...
$a->strings["Contact unfollowed"] = "Вы отписались от контакта";
...
$a->strings["Welcome back %s"] = "Добро пожаловать, %s";
...
$a->strings["Post update version number has been set to %s."] = "Номер версии обновления записи установлен на %s.";
...
$a->strings["Network Sidebar"] = "Панель Сеть";
$a->strings["Archives"] = "Архивы";
$a->strings["Ability to select posts by date ranges"] = "Возможность выбора записей по диапазону дат";
$a->strings["Protocol Filter"] = "Фильтр протоколов";
$a->strings["Enable widget to display Network posts only from selected protocols"] = "Включить возможность фильтрации записей по протоколам на панели Сеть";
...
$a->strings["Following"] = "Подписчики";
...
$a->strings["If you don't have a command line version of PHP installed on your server, you will not be able to run the background processing. See <a href='https://github.com/friendica/friendica/blob/master/doc/Install.md#set-up-the-worker'>'Setup the worker'</a>"] = "Если у вас нет доступа к командной строке PHP  на вашем сервере, вы не сможете использовать фоновые задания. Посмотрите <a href='https://github.com/friendica/friendica/blob/master/doc/Install.md#set-up-the-worker'>'Настройка фоновых заданий'</a>";
...
$a->strings["Database storage failed to update %s"] = "Хранилищу БД не удалось обновить %s";
$a->strings["Database storage failed to insert data"] = "Хранилищу БД не удалось записать данные";
$a->strings["Filesystem storage failed to create \"%s\". Check you write permissions."] = "Файловому хранилищу не удалось создать \"%s\". Проверьте, есть ли у вас разрешения на запись.";
$a->strings["Filesystem storage failed to save data to \"%s\". Check your write permissions"] = "Файловому хранилищу не удалось записать данные в \"%s\". Проверьте, есть ли у вас разрешения на запись.";
...
$a->strings["Site blocklist updated."] = "Черный список узлов обновлён.";
...
$a->strings["The <em>Auto Discovered Contact Directory</em> feature is not enabled, it will improve the data displayed here."] = "<em>Автообнаружение контактов</em> не включено, эта функция улучшила бы отображаемую здесь статистику.";
...
$a->strings["none"] = "нет";
$a->strings["Direct contacts"] = "Прямые контакты";
$a->strings["Contacts of contacts"] = "Контакты контактов";
...
$a->strings["Allowed OEmbed domains"] = "Разрешённые OEmbed домены ";
$a->strings["Comma separated list of domains which oembed content is allowed to be displayed. Wildcards are accepted."] = "Список доменов через запятую, содержимое oembed с них будет отображаться. Можно использовать маски.";
...
$a->strings["Disable OpenID"] = "Отключить OpenID";
$a->strings["Disable OpenID support for registration and logins."] = "Отключить поддержку OpenID для регистрации и входа.";
$a->strings["No Fullname check"] = "Не проверять полное имя";
$a->strings["Allow users to register without a space between the first name and the last name in their full name."] = "Разрешить пользователям регистрироваться, если указанное ими имя не имеет пробела между именем и фамилией.";
...
$a->strings["Disable OStatus support"] = "Отключить поддержку OStatus";
$a->strings["Disable built-in OStatus (StatusNet, GNU Social etc.) compatibility. All communications in OStatus are public, so privacy warnings will be occasionally displayed."] = "Отключить встроенную поддержку OStatus (StatusNet, GNU Social и т.п.). Всё общение в OStatus происходит публично, поэтому возможны периодические предупреждения о приватности.";
$a->strings["OStatus support can only be enabled if threading is enabled."] = "Поддержка OStatus может быть включена только вместе с поддержкой веток диалогов.";
...
$a->strings["Maximum table size for optimization"] = "Максимальный размер таблицы для оптимизации";
$a->strings["Maximum table size (in MB) for the automatic optimization. Enter -1 to disable it."] = "Максимальный размер таблицы (в MB) для автоматической оптимизации. Введите -1, чтобы отключить это.";
$a->strings["Minimum level of fragmentation"] = "Минимальная фрагментация";
$a->strings["Minimum fragmenation level to start the automatic optimization - default value is 30%."] = "Минимальный уровень фрагментации для автоматической оптимизации - по умолчанию 30%.";
$a->strings["Periodical check of global contacts"] = "Периодически проверять глобальные контакты";
$a->strings["If enabled, the global contacts are checked periodically for missing or outdated data and the vitality of the contacts and servers."] = "Если включено, глобальные контакты периодически проверяются для актуализации данных и проверки жизнеспособности контактов и серверов.";
$a->strings["Discover followers/followings from global contacts"] = "Обнаруживать подписки среди глобальных контактов";
$a->strings["If enabled, the global contacts are checked for new contacts among their followers and following contacts. This option will create huge masses of jobs, so it should only be activated on powerful machines."] = "Если включено, у глобальных контактов будут так же проверяться их новые подписки и подписчики. Эта настройка создаст очень много заданий, поэтому её имеет смысл включать на мощных серверах.";
...
$a->strings["Timeframe for fetching global contacts"] = "Период активности глобальных контактов";
$a->strings["When the discovery is activated, this value defines the timeframe for the activity of the global contacts that are fetched from other servers."] = "Когда обнаружение включено, это значение определяет период активности, за который глобальные контакты загружаются с удалённых серверов.";
...
$a->strings["The item caches buffers generated bbcode and external images."] = "Кэш записей хранит сгенерированные элементы BBCode и внешние изображения.";
$a->strings["Cache duration in seconds"] = "Время жизни кэша в секундах";
$a->strings["How long should the cache files be hold? Default value is 86400 seconds (One day). To disable the item cache, set the value to -1."] = "Как долго кэш должен хранить содержимое? Значение по умолчанию 86400 секунд (один день). Чтобы отключить, установите значение -1.";
...
$a->strings["The picture proxy increases performance and privacy. It shouldn't be used on systems with very low bandwidth."] = "Прокси-сервер изображений улучшает производительность и приватность. Его можно выключить для систем с сильно ограниченной пропускной полосой.";
...
$a->strings["Subscribe to relay"] = "Подписаться на ретранслятор";
$a->strings["Enables the receiving of public posts from the relay. They will be included in the search, subscribed tags and on the global community page."] = "Включает получение публичных записей через ретранслятор. Они будут использоваться в результатах поиска, подписках на тэги и на общей публичной ленте.";
$a->strings["Relay server"] = "Сервер ретрансляции";
$a->strings["Address of the relay server where public posts should be send to. For example https://relay.diasp.org"] = "Адрес сервера ретрансляции, куда будут отсылаться публичные записи. Например https://relay.diasp.org";
...
$a->strings["The Terms of Service settings have been updated."] = "Настройки Условий Оказания Услуг были обновлены.";
...
$a->strings["Submanaged account can't access the administation pages. Please log back in as the master account."] = "При делегировании доступ к странице администратора невозможен. Пожалуйста, зайдите под учётной записью администратора напрямую.";
...
$a->strings["Search term successfully saved."] = "Поисковый запрос сохранён.";
$a->strings["Search term already saved."] = "Такой запрос уже сохранён.";
$a->strings["Search term successfully removed."] = "Сохранённый запрос успешно удалён.";
...
$a->strings["Disable Smart Threading"] = "Отключить умное ветвление";
$a->strings["Disable the automatic suppression of extraneous thread indentation."] = "Отключить автоматическое удаление излишних отступов в ветках диалогов.";
$a->strings["Hide the Dislike feature"] = "Убрать функцию \"Не нравится\"";
$a->strings["Hides the Dislike button and dislike reactions on posts and comments."] = "Убирает кнопку \"Не нравится\" и отображение реакции \"не нравится\" в постах и комментариях.";
